...
impl State {
    pub fn new() -> Self {
        Self {
            cam_start_pos: at_origin(Vector3::new(0., 3., -10.)),
            cam_start_tgt: at_origin(Vector3::new(-8., 0., 0.)),
...
impl Example for State {
    ...
    fn update(&mut self, args: Args) -> Vec<(Shape, Color)> {
        ...
        for i in 0..16 {
            let x = (i as f32) - 16.;
            let y = -2. + i as f32 * 0.25;

            let tpos = [
                at_origin(Vector3::new(x + 2., y, -2.)),
                at_origin(Vector3::new(x + -2., y, -2.)),
                at_origin(Vector3::new(x + -2., y, 2.)),
            ];
            surfs.push(Surface::new(
                [
                    tpos[0].to_mcapv3(),
                    tpos[1].to_mcapv3(),
                    tpos[2].to_mcapv3(),
            ...
            out.push((
                Shape::Triangle(tpos),
                Color::GRAY.lerp(Color::WHITE, i as f32 / 10.),
            ));
        ...
        let base = center + Vec3::ZERO.with_x(-8. + 10. * (args.time / 4.).sin() as f32);
    ...
    fn draw_2d(&mut self, _args: Args, mut d: RaylibDrawHandle<'_>) {
        ...
        d.draw_text(&format!("E. floor stepping"), 20, 20, 20, Color::BLACK);
        d.draw_text(
            &format!(
                "p1: {:.1} {:.1} {:.1}",
