...
use mcap::{
    HotDog, Surface, Vec3, get_face_normal
};
use raylib::prelude::*;
...
fn main() {
    ...
    while !rl.window_should_close() {
        ...
        let mut d = rl.begin_drawing(&thread);
        {
            ...
            let clips: Vec<_> = walls
                .iter()
                .map(|wall| {
                    hotdog.clip_line_segment(
                        Vec2::new(wall.p1.x as f32, wall.p1.y as f32),
                        Vec2::new(wall.p2.x as f32, wall.p2.y as f32),
                    )
                })
                .collect();
            ...
            let cp1 = hotdog.closest_point_on_segment_origin(
                Vec2::new(p1.x as f32, p1.y as f32),
                Vec2::new(p2.x as f32, p2.y as f32),
            );
            d.draw_circle(cp1.x as i32, cp1.y as i32, 3., Color::GREEN);
            let cp2 = hotdog.closest_point_on_segment_origin(
                Vec2::new(p2.x as f32, p2.y as f32),
                Vec2::new(p3.x as f32, p3.y as f32),
            );
            d.draw_circle(cp2.x as i32, cp2.y as i32, 3., Color::GREEN);
            let cp3 = hotdog.closest_point_on_segment_origin(
                Vec2::new(p3.x as f32, p3.y as f32),
                Vec2::new(p1.x as f32, p1.y as f32),
            );
            ...
            let vrect = Rectangle::new(top_left.x, top_left.y, rect_width, rect_height);
            ...
            for clip in clips {
                if let Some((p1, p2)) = clip {
                    ...
                    // closest point rect
                    let cp1 = hotdog.closest_point_on_segment_rect_circ(
                        Vec2::new(p1.x as f32, p1.y as f32),
                        Vec2::new(p2.x as f32, p2.y as f32),
                    );
                    match nearest {
                    ...
                    d.draw_circle_lines(
                        (top_left.x + radius + cp1.x) as i32,
                        (top_left.y + cp1.y) as i32,
                        3.,
                        Color::GREEN,
                    );

                    let true_closest = hotdog.closest_point_on_segment_rect(
                        Vec2::new(p1.x as f32, p1.y as f32),
                        Vec2::new(p2.x as f32, p2.y as f32),
                    );
                    d.draw_circle(
                        (top_left.x + radius + true_closest.x) as i32,
                        (top_left.y + true_closest.y) as i32,
                        3.,
                        Color::GREEN,
                    );
                }
            ...
            if let Some(hdc) = res {
                ...
                d.draw_line_ex(
                    p_stop_v2,
                    p_stop_v2 + Vector2::new(hdc.next_move.x, hdc.next_move.y),
                    3.,
                    Color::HOTPINK,
                );
            ...
            if let Some(n) = nearest {
                ...
                d.draw_line(
                    ...
                    top_left.y as i32,
                    (top_left.x + radius + n.x) as i32,
                    (top_left.y + n.y) as i32,
                    Color::BLACK,
                );
                d.draw_circle_lines(
                    (top_left.x + radius + n.x) as i32,
